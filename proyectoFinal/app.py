...
@app.route('/unidadestadistica', methods=['GET', 'POST'])
def unidadestadistica():
    if request.method == 'POST':
    ...
    return render_template('printMediaMedianaModa.html')


@app.route('/promedioMovil', methods=['GET', 'POST'])
def promedioMovil():
    if request.method == 'POST':
        file = request.files['file'].read()
        tipoArch= request.form.get("tipoarchivo")
        ...
        df = movil.append({columna1: columna1, columna2: p1,'MMO_3': p2, 'MMO_4': p3}, ignore_index=True)
        ...
        fig.clear()
        plot_url = base64.b64encode(buf.getvalue()).decode('UTF-8')
        data = df.to_html(classes="table table-hover table-striped",justify="justify-all", border=0)
...
@app.route('/suavizacionExponencial', methods=['GET', 'POST'])
def suavizacionExponencial():
    ...
    return render_template('printSuavizacionExponencial.html')
...
@app.route('/regresionLinealCuadrada', methods=['GET', 'POST'])
def regresionLinealCuadrada():
    ...
    return render_template('printRegresionLinealCuadrada.html')
...
@app.route('/sistemaInventario', methods=['GET', 'POST'])
def sistemaInventario():
    if request.method == 'POST':
        ...
        data2 = dfQ.to_html(classes="table table-hover table-striped", justify="justify-all", border=0)
...
@app.route('/sistemaLineaEspera', methods=['GET', 'POST'])
